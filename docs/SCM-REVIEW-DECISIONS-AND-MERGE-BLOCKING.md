...
**Optional reply-dismissal.** If turned on in configuration (**§2**), an extra step can load the **PR review-comment** thread, run a small LLM, and—if it judges the reply sufficient—**omit that thread from counts for this run**. On **GitHub** and **GitLab**, the runner also resolves the thread in the SCM; on other supported SCMs, exclusion remains run-local only. If it judges the reply insufficient, the runner may post a brief follow-up **on that comment thread** where the API allows.
