...
| **Develop or test locally** (any SCM) | **[Development testing](docs/DEV_TESTING.md)** — run `code-review` directly; **[Developer guide](docs/DEVELOPER_GUIDE.md)** — architecture and extension points; **[Valuation harness](docs/VALUATION-HARNESS.md)** — eval corpus, CI gate, and baseline workflow. |
...
**Reply-dismissal (review-decision-only)** — When you run **`--review-decision-only`** with webhook context (`CODE_REVIEW_EVENT_*`), a reply on a review thread is classified by default so **agreed** threads are excluded from the quality gate for that run and, on **GitHub** and **GitLab**, the thread is also resolved in the SCM; **disagreed** may post a follow-up on **GitHub**, **GitLab**, **Bitbucket Cloud**, or **Bitbucket Server / DC** where supported. Set **`CODE_REVIEW_REPLY_DISMISSAL_ENABLED=false`** to disable it. See [Configuration reference](docs/CONFIGURATION-REFERENCE.md) §5.
