...
- `@ts-ignore` / `@ts-expect-error` suppressing real errors: flag `// @ts-ignore` comments that silence a genuine type error rather than documenting a known, intentional edge case. Unlike `@ts-expect-error` (which fails when no error is present), `@ts-ignore` silently masks issues forever, including new type errors introduced later. Prefer fixing the type mismatch; if suppression is unavoidable, use `@ts-expect-error` with a comment explaining why.
- Async callback in `Array.prototype.forEach`: flag `array.forEach(async (item) => { ... })`. TypeScript types `forEach`'s callback return as `void`, so the returned `Promise` is discarded; rejections inside the callback are silently swallowed and the loop does not wait for async work to finish. Use `Promise.all(array.map(async ...))` or a `for...of` loop with `await`.
